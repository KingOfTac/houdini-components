import type { Coords, Point, PaintSize } from '@kingoftac/worklet-utilities';
...
class PolygonPainter {
	...
	paint(context:CanvasRenderingContext2D, geometry:PaintSize, properties:any) {
		const config: any = {};
		props.propsMap.forEach((prop:any) => config[prop.name] = prop.value(properties.get(prop.variable)));
